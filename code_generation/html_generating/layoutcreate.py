from code_generation.html_generating.shapes.shapes4 import input_array
...
#if same y value then adjust with x values
    count3 = 0
    kk = []
    i = 0
   ...
   # print("classified:", sorted_list)
        ...
        if (sorted_list[n][0][1]) == 'RB':
            ...
            for m in range(0, len(sorted_list)):
                if ((sorted_list[m][1][0] > left_point) and (sorted_list[m][1][4] < right_point) and (
                        sorted_list[m][0][1] == 'LBL') and (sorted_list[m][1][5] < sorted_list[n][1][1])):
    ...
    for n in range(0, len(sorted_list)):
        if (len(sorted_list[n][0]) == 2):
            if ((sorted_list[n][0][1] == 'TEXT') or (sorted_list[n][0][1] == 'PW')):
                ...
                if (label_count != input_count):
                    for m in range(n + 1, len(sorted_list)):
                        ...
                        if (sorted_list[m][0][1] == 'DD'):
                            break
                        if ((sorted_list[m][0][1] == 'TEXT') or (sorted_list[m][0][1] == 'PW')):
                            sorted_list.insert(n + 1, sorted_list.pop(m))
                            sorted_list[n][0].append('classified')
                            break
                        else:
    ...
    group_id = 1
    for n in range(0, len(sorted_list)):
        if (sorted_list[n][0][1] == 'IMG') or (sorted_list[n][0][1] == 'HPL') or (sorted_list[n][0][1] == 'PRGF') or (
                sorted_list[n][0][1] == 'BTN'):
            sorted_list[n].append(group_id)
            group_id = group_id + 1
        # checkbox and label or paragraph
        if (sorted_list[n][0][1] == 'CB') and ((sorted_list[n + 1][0][1] == 'LBL') or (sorted_list[n + 1][0][1] == 'PRGF')):
            sorted_list[n].append(group_id)
            group_id = group_id + 1
        # label and text, password, paragraph
        if (sorted_list[n][0][1] == 'LBL') and (
                (sorted_list[n + 1][0][1] == 'TEXT') or (sorted_list[n + 1][0][1] == 'PRGF') or (
                sorted_list[n + 1][0][1] == 'PW')):
            sorted_list[n].append(group_id)
            for m in range(n + 1, len(sorted_list)):
                if ((sorted_list[m][0][1] == 'TEXT') or (sorted_list[m][0][1] == 'PW') or (sorted_list[m][0][1] == 'PRGF')):
        ...
        if ((sorted_list[n][0][1] == 'LBL') and (len(sorted_list[n][0]) == 2) and (sorted_list[n + 1][0][1] == 'RB')):
            sorted_list[n].append(group_id)
            for m in range(n + 1, len(sorted_list)):

                if ((sorted_list[m][0][1] == 'TEXT') or (sorted_list[m][0][1] == 'PW') or (
...
def htmlGeneration(shapes):
    ...
    for page_number in range(0, len(shapes)):
        ...
        increase_ratio = (1366 / page_width) * 100   ### 1366 - most popular screen width
